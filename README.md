...
The authenticated workspace release adds invitation-only authentication, tenant authorization, editable assets/search, maintenance/readings, server-saved drafts and audit. Identity/runtime infrastructure and live migrations are applied, and actual Aurora integration passed; the application release is in progress. Account creation and invitations remain deferred. See the [workspace scope and runbook](docs/AUTHENTICATED-WORKSPACE.md) and [current deployment record](docs/DEPLOYMENT-2026-09-25-AUTH.md).
